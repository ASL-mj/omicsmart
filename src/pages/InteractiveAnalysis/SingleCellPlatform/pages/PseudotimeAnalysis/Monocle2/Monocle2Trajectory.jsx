...
const Monocle2Trajectory = () => {
  ...
  const differentiationChartRef = useRef(null);
  ...
  const [selectedDifferentiationRows, setSelectedDifferentiationRows] = useState(['1', '2', '3', '4', '5']);
  ...
  const pseudotimeTableData = [
    { key: '1', cell: 'Cell_001', x: 2.3, y: 1.8, pseudotime: 0.23, state: 'State_1' },
    { key: '2', cell: 'Cell_002', x: -1.2, y: 0.5, pseudotime: 0.45, state: 'State_2' },
    { key: '3', cell: 'Cell_003', x: 0.8, y: -2.1, pseudotime: 0.67, state: 'State_2' },
    { key: '4', cell: 'Cell_004', x: 1.5, y: 0.9, pseudotime: 0.82, state: 'State_3' },
    { key: '5', cell: 'Cell_005', x: -0.7, y: 1.6, pseudotime: 0.35, state: 'State_1' },
  ];
  ...
  // 分化状态信息表格数据
  const differentiationTableData = [
    { key: '1', cell: 'Cell_001', x: 2.3, y: 1.8, state: 'Early', score: 0.15 },
    { key: '2', cell: 'Cell_002', x: -1.2, y: 0.5, state: 'Mid', score: 0.52 },
    { key: '3', cell: 'Cell_003', x: 0.8, y: -2.1, state: 'Late', score: 0.87 },
    { key: '4', cell: 'Cell_004', x: 1.5, y: 0.9, state: 'Mid', score: 0.63 },
    { key: '5', cell: 'Cell_005', x: -0.7, y: 1.6, state: 'Early', score: 0.28 },
  ];

  const differentiationTableColumns = [
    { title: '细胞', dataIndex: 'cell', key: 'cell' },
    { title: 'X坐标', dataIndex: 'x', key: 'x', sorter: (a, b) => a.x - b.x },
    { title: 'Y坐标', dataIndex: 'y', key: 'y', sorter: (a, b) => a.y - b.y },
  ...
  const sampleTableData = [
    { key: '1', cell: 'Cell_001', x: 2.3, y: 1.8, sample: 'Sample_A', cluster: 'Cluster_1' },
    { key: '2', cell: 'Cell_002', x: -1.2, y: 0.5, sample: 'Sample_B', cluster: 'Cluster_2' },
    { key: '3', cell: 'Cell_003', x: 0.8, y: -2.1, sample: 'Sample_B', cluster: 'Cluster_2' },
    { key: '4', cell: 'Cell_004', x: 1.5, y: 0.9, sample: 'Sample_C', cluster: 'Cluster_3' },
    { key: '5', cell: 'Cell_005', x: -0.7, y: 1.6, sample: 'Sample_A', cluster: 'Cluster_1' },
  ];
  ...
  const clusterTableData = [
    { key: '1', cell: 'Cell_001', x: 2.3, y: 1.8, cluster: 'Epithelial', conf: 0.92 },
    { key: '2', cell: 'Cell_002', x: -1.2, y: 0.5, cluster: 'Stromal', conf: 0.85 },
    { key: '3', cell: 'Cell_003', x: 0.8, y: -2.1, cluster: 'Immune', conf: 0.78 },
    { key: '4', cell: 'Cell_004', x: 1.5, y: 0.9, cluster: 'Epithelial', conf: 0.94 },
    { key: '5', cell: 'Cell_005', x: -0.7, y: 1.6, cluster: 'Stromal', conf: 0.87 },
  ];
  ...
  // 更新分化状态信息散点图 (D3)
  const updateDifferentiationChart = (selectedKeys) => {
    if (!differentiationChartRef.current) return;

    // 清空之前的内容
    d3.select(differentiationChartRef.current).selectAll('*').remove();

    // 根据选中的行过滤数据
    const selectedData = differentiationTableData.filter(item => selectedKeys.includes(item.key));
    
    if (selectedData.length === 0) return;

    // 设置尺寸和边距
    const margin = { top: 60, right: 120, bottom: 50, left: 60 };
    const width = differentiationChartRef.current.clientWidth - margin.left - margin.right;
    const height = 500 - margin.top - margin.bottom;

    // 创建 SVG
    const svg = d3.select(differentiationChartRef.current)
      .append('svg')
      .attr('width', width + margin.left + margin.right)
      .attr('height', height + margin.top + margin.bottom)
      .append('g')
  ...
  // 初始化分化状态信息图 - 当 Tab 切换到分化状态信息时初始化
  useEffect(() => {
    if (activeTab === 'differentiation' && differentiationChartRef.current) {
      updateDifferentiationChart(selectedDifferentiationRows);
      
      // 响应式
      const handleResize = () => updateDifferentiationChart(selectedDifferentiationRows);
      window.addEventListener('resize', handleResize);
  ...
  }, [activeTab, selectedDifferentiationRows]);
  ...
  const handleDifferentiationDownload = (selectedRowKeys) => {
    message.success(`正在下载分化状态信息的 ${selectedRowKeys.length} 条数据`);
  };
  ...
  const handleDifferentiationRefresh = () => {
    message.success('已刷新分化状态信息数据');
    setSelectedDifferentiationRows(['1', '2', '3', '4', '5']);
  };
  ...
  return (
    <PageTemplate pageTitle="细胞轨迹图">
      ...
      <div style={{ display: activeTab === 'differentiation' ? 'block' : 'none' }}>
        <ResultPageTemplate 
          chartContent={
            <div 
              ref={differentiationChartRef}
              style={{ 
                width: '100%',
                height: '500px',
                background: '#fff',
          ...
          tableData={differentiationTableData}
          selectedRowKeys={selectedDifferentiationRows}
          onSelectChange={setSelectedDifferentiationRows}
          onDownload={handleDifferentiationDownload}
          onRefresh={handleDifferentiationRefresh}
        />
      </div>
