...
const SlingshotPseudotimeMapping = () => {
  ...
  // 初始化拟时间轨迹信息图 - 当 Tab 切换到拟时间轨迹信息时初始化
  useEffect(() => {
  ...
  const handleTrajectoryDownload = (selectedRowKeys) => {
    message.success(`正在下载拟时间轨迹信息的 ${selectedRowKeys.length} 条数据`);
  };

  // 刷新处理
  const handleTrajectoryRefresh = () => {
    message.success('已刷新拟时间轨迹信息数据');
...
export default SlingshotPseudotimeMapping;