...
const SlingshotDimensionReduction = () => {
  ...
  // 选中的行
  const [selectedClusteringRows, setSelectedClusteringRows] = useState(['1', '2', '3', '4', '5']);

  // 分群信息图表数据
  const clusteringTableData = [
  ...
  // 更新分群信息散点图 (D3)
  const updateClusteringChart = (selectedKeys) => {
    if (!clusteringChartRef.current) return;

    // 清空之前的内容
    d3.select(clusteringChartRef.current).selectAll('*').remove();

    // 根据选中的行过滤数据
    const selectedData = clusteringTableData.filter(item => selectedKeys.includes(item.key));
    
    if (selectedData.length === 0) return;

    // 设置尺寸和边距
    const margin = { top: 60, right: 120, bottom: 50, left: 60 };
    const width = clusteringChartRef.current.clientWidth - margin.left - margin.right;
    const height = 500 - margin.top - margin.bottom;

    // 创建 SVG
    const svg = d3.select(clusteringChartRef.current)
      .append('svg')
  ...
  // 初始化分群信息图 - 当 Tab 切换到分群信息时初始化
  useEffect(() => {
    if (activeTab === 'clustering' && clusteringChartRef.current) {
      updateClusteringChart(selectedClusteringRows);
      
      // 响应式
      const handleResize = () => updateClusteringChart(selectedClusteringRows);
  ...
  }, [activeTab, selectedClusteringRows]);

  // 下载处理
  const handleClusteringDownload = (selectedRowKeys) => {
    message.success(`正在下载分群信息的 ${selectedRowKeys.length} 条数据`);
  };

  // 刷新处理
  const handleClusteringRefresh = () => {
    message.success('已刷新分群信息数据');
    setSelectedClusteringRows(['1', '2', '3', '4', '5']);
  };
  ...
  return (
    <PageTemplate pageTitle="Slingshot降维图">
      <Tabs
        activeKey={activeTab}
      ...
      <div style={{ display: activeTab === 'clustering' ? 'block' : 'none' }}>
        <ResultPageTemplate 
          ...
          tableColumns={clusteringTableColumns}
          tableData={clusteringTableData}
          selectedRowKeys={selectedClusteringRows}
          onSelectChange={setSelectedClusteringRows}
          onDownload={handleClusteringDownload}
          onRefresh={handleClusteringRefresh}
        />
      </div>
    </PageTemplate>
  );
