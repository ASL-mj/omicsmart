...
const Monocle3Mapping = () => {
  ...
  const pseudotimeMapChartRef = useRef(null);
  const partitionMapChartRef = useRef(null);
  ...
  const pseudotimeTableData = [
    { key: '1', cell: 'Cell_001', x: 2.3, y: 1.8, pseudotime: 0.23, partition: 'Partition_A', cluster: 'Cluster_1' },
    { key: '2', cell: 'Cell_002', x: -1.2, y: 0.5, pseudotime: 0.45, partition: 'Partition_B', cluster: 'Cluster_2' },
    { key: '3', cell: 'Cell_003', x: 0.8, y: -2.1, pseudotime: 0.67, partition: 'Partition_A', cluster: 'Cluster_2' },
    { key: '4', cell: 'Cell_004', x: 1.5, y: 0.9, pseudotime: 0.82, partition: 'Partition_C', cluster: 'Cluster_3' },
    { key: '5', cell: 'Cell_005', x: -0.7, y: 1.6, pseudotime: 0.35, partition: 'Partition_B', cluster: 'Cluster_1' },
  ];
  ...
  const partitionTableData = [
    { key: '1', cell: 'Cell_001', x: 2.3, y: 1.8, partition: 'Partition_A', pseudotime: 0.23, cluster: 'Epithelial' },
    { key: '2', cell: 'Cell_002', x: -1.2, y: 0.5, partition: 'Partition_B', pseudotime: 0.45, cluster: 'Stromal' },
    { key: '3', cell: 'Cell_003', x: 0.8, y: -2.1, partition: 'Partition_A', pseudotime: 0.67, cluster: 'Immune' },
    { key: '4', cell: 'Cell_004', x: 1.5, y: 0.9, partition: 'Partition_C', pseudotime: 0.82, cluster: 'Epithelial' },
    { key: '5', cell: 'Cell_005', x: -0.7, y: 1.6, partition: 'Partition_B', pseudotime: 0.35, cluster: 'Stromal' },
  ];
  ...
  // 更新拟时间映射图 (D3)
  const updatePseudotimeMapChart = (selectedKeys) => {
    if (!pseudotimeMapChartRef.current) return;

    // 清空之前的内容
    d3.select(pseudotimeMapChartRef.current).selectAll('*').remove();
    ...
    const width = pseudotimeMapChartRef.current.clientWidth - margin.left - margin.right;
    const height = 500 - margin.top - margin.bottom;

    // 创建 SVG
    const svg = d3.select(pseudotimeMapChartRef.current)
      .append('svg')
  ...
  // 更新分区映射图 (D3)
  const updatePartitionMapChart = (selectedKeys) => {
    if (!partitionMapChartRef.current) return;

    // 清空之前的内容
    d3.select(partitionMapChartRef.current).selectAll('*').remove();
    ...
    // 设置尺寸和边距
    const margin = { top: 60, right: 120, bottom: 50, left: 60 };
    const width = partitionMapChartRef.current.clientWidth - margin.left - margin.right;
    const height = 500 - margin.top - margin.bottom;

    // 创建 SVG
    const svg = d3.select(partitionMapChartRef.current)
      .append('svg')
      .attr('width', width + margin.left + margin.right)
      .attr('height', height + margin.top + margin.bottom)
      .append('g')
  ...
  // 初始化拟时间映射图 - 当 Tab 切换到拟时间映射图时初始化
  useEffect(() => {
    if (activeTab === 'pseudotime' && pseudotimeMapChartRef.current) {
      updatePseudotimeMapChart(selectedPseudotimeRows);
      
      // 响应式
      const handleResize = () => updatePseudotimeMapChart(selectedPseudotimeRows);
  ...
  // 初始化分区映射图 - 当 Tab 切换到分区映射图时初始化
  useEffect(() => {
    if (activeTab === 'partition' && partitionMapChartRef.current) {
      updatePartitionMapChart(selectedPartitionRows);
      
      // 响应式
      const handleResize = () => updatePartitionMapChart(selectedPartitionRows);
      window.addEventListener('resize', handleResize);
  ...
  const handlePseudotimeDownload = (selectedRowKeys) => {
    message.success(`正在下载拟时间映射图的 ${selectedRowKeys.length} 条数据`);
  };

  const handlePartitionDownload = (selectedRowKeys) => {
    message.success(`正在下载分区映射图的 ${selectedRowKeys.length} 条数据`);
  };

  // 刷新处理
  const handlePseudotimeRefresh = () => {
    message.success('已刷新拟时间映射图数据');
    setSelectedPseudotimeRows(['1', '2', '3', '4', '5']);
  };

  const handlePartitionRefresh = () => {
    message.success('已刷新分区映射图数据');
  ...
  return (
    <PageTemplate pageTitle="Monocle3映射图">
      <Tabs
        activeKey={activeTab}
